...
let tiempoHastaPajaro = 3; // Tiempo hasta la aparición del siguiente pajaros
let tiempoPajaroMin = 0.9; // Tiempo mínimo entre pajaros
let tiempoPajaroMax = 2.1; // Tiempo máximo entre pajaros
let maxPajaroY = 100; // Altura máxima de los pajaros
let minPajaroY = 25; // Altura mínima de los pajaros
let pajaros = []; // Array para almacenar los pajaros
let velPajaro = 0.7; // Velocidad de los pajaros
let pajaro_active = false;
...
// Decide si debe crear un nuevo pájaro
function DecidirCrearPajaros() {

    if(!pajaro_active) return;
    tiempoHastaPajaro -= deltaTime; // Reduce el tiempo hasta el próximo pájaro
